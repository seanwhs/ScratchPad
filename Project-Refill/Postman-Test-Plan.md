...
|                   | `/inventories/{{INVENTORY_ID}}/`              | PUT    | `json {"depot": null, "customer": 1, "equipment": 2, "quantity": 10}`                                                         | ✅          | Full update                         | `pm.test("Inventory updated", () => {
    pm.response.to.have.status(200);
    const json = pm.response.json();
    pm.expect(json.quantity).to.eql(pm.request.body.raw.includes('"quantity": 10') ? 10 : 5);
});
`                                                                                        |
